...
export function WeatherView({ weather }: WeatherViewProps) {
  ...
  return (
    <div className="py-6 md:py-12 px-2 max-w-7xl mx-auto animate-fade-in">
      {/* Hero Weather Section */}
      <section className="relative rounded-[28px] md:rounded-[40px] overflow-hidden mb-10 md:mb-16 botanical-gradient text-white shadow-2xl p-5 md:p-12 min-h-[460px] md:min-h-[400px] flex flex-col justify-end">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-12 items-end">
          <div className="space-y-4 md:space-y-6">
            <div>
              <h2 className="text-[11px] md:text-sm font-bold uppercase tracking-[0.16em] md:tracking-widest mb-2 opacity-80 flex items-center gap-2 leading-relaxed">
                Điều kiện khí quyển địa phương
              </h2>
              <h1 className="text-5xl sm:text-6xl md:text-8xl leading-none font-headline font-bold">{weather.city}</h1>
            </div>
            
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 md:gap-8">
              <span className="text-6xl md:text-7xl font-sans font-light">{weather.temp}°C</span>
              <div className="hidden sm:block h-14 md:h-16 w-px bg-white/20"></div>
              <div className="min-w-0">
                <p className="text-3xl md:text-2xl font-bold uppercase tracking-wide leading-tight">{weather.condition}</p>
                <p className="text-sm opacity-80 flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 font-medium">
      ...
      <section className="space-y-6 md:space-y-10">
        <div className="flex flex-col gap-3 md:flex-row md:items-baseline md:justify-between border-b border-outline-variant/20 pb-4 md:pb-6">
          <h2 className="text-2xl md:text-4xl font-headline text-primary font-bold">Hành động bảo vệ thực vật</h2>
          <span className="text-xs md:text-sm font-bold text-secondary uppercase tracking-widest px-4 py-1 bg-secondary-container rounded-full w-fit">Nhiệm vụ ưu tiên</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-8">
          {/* Main Action Card */}
          <div className="md:col-span-2 bg-surface-container-low rounded-[28px] md:rounded-[40px] p-5 md:p-10 flex flex-col md:flex-row gap-5 md:gap-10 items-center border border-outline-variant/10 hover:border-primary/20 transition-all group">
            ...
            <div className="flex-grow space-y-4 md:space-y-6">
              ...
              <h3 className="text-2xl md:text-3xl font-headline font-bold text-primary leading-tight">{actions[0].title}</h3>
              <p className="text-on-surface-variant leading-relaxed text-base md:text-lg font-medium">{actions[0].desc}</p>
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-2 md:pt-4">
                <button className="bg-primary text-on-primary px-6 md:px-8 py-3 rounded-xl font-bold text-sm shadow-lg shadow-primary/20 hover:opacity-90 transition-opacity">Đã hoàn thành</button>
                <button className="text-primary px-6 md:px-8 py-3 rounded-xl font-bold text-sm bg-white border border-outline-variant hover:bg-surface-container transition-colors">Xem chi tiết</button>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-5 md:space-y-8">
            {actions.slice(1).map((action, idx) => {
              ...
              return (
                <div key={idx} className="bg-surface-container-lowest rounded-[24px] md:rounded-[32px] p-5 md:p-8 space-y-4 md:space-y-6 border border-outline-variant/10 shadow-sm relative overflow-hidden group">
                  ...
                  <h3 className="text-xl md:text-2xl font-headline font-bold text-primary">{action.title}</h3>
      ...
      {/* Insights */}
      <section className="mt-12 md:mt-24 text-center max-w-2xl mx-auto py-8 md:py-12 px-5 md:px-6 bg-surface-container-low rounded-[28px] md:rounded-[40px] border border-outline-variant/10">
        <p className="text-xl md:text-2xl font-headline italic text-primary leading-relaxed mb-6 font-medium">
